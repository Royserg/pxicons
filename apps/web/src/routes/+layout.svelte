<script lang="ts">
	...
	const isLockedBrowserRoute = $derived(
		page.url.pathname === '/icons' || page.url.pathname === '/icons/lucide'
	);
	...
	onMount(async () => {
		try {
			...
			if (
				...
				githubStars = payload.stargazers_count.toLocaleString();
			}
		} catch {
...
<div class={`flex w-full flex-col ${isLockedBrowserRoute ? 'h-dvh overflow-hidden' : 'min-h-dvh'}`}>
	<header class="mx-auto flex w-full max-w-325 items-center justify-between gap-4 px-1">
		<a href="/" class="group flex items-center justify-start">
	...
	<main
		class={`mx-auto flex min-h-0 w-full max-w-310 flex-1 flex-col gap-4 p-4 py-2 ${isLockedBrowserRoute ? 'overflow-hidden' : ''}`}
	>
		{@render children()}
	</main>
</div>
