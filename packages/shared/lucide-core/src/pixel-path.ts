import type { IconPixels, IconRects, IconRenderMode, PixelShape } from './types.ts';
...
export function createSquareSubpath(x: number, y: number, width: number, height = width): string {
...
export function createCircleSubpath(x: number, y: number, size: number): string {
  const radius = size / 2;
...
export function createRoundedSubpath(x: number, y: number, size: number): string {
